...
const RACE_COLORS = {
  white: '#1a5bd6', black: '#d64545', hispanic: '#f5b02e',
  asian: '#0f9d6c', aian: '#8e44ad', two_plus: '#378add', other: '#555550',
}
const RACE_LABELS = {
  white: 'White', black: 'Black', hispanic: 'Hispanic',
  asian: 'Asian', aian: 'Native American', two_plus: 'Two+ Races', other: 'Other',
}
          ...
          {ageGroups.map(ag => (
            <div className="panel-bar-row" key={ag.label}>
              ...
              <div className="panel-bar-track">
                <div className="panel-bar-fill" style={{ width: `${(ag.value / maxAge) * 100}%` }} />
              </div>
              <span className="panel-bar-value">{((ag.value / state.age.total) * 100).toFixed(1)}%</span>
            </div>
          ))}
        </div>
        ...
        <div className="race-list">
          {raceGroups.map(r => (
            <div className="race-row" key={r.key}>
              ...
              <span className="race-pct">{((r.value / state.race.total) * 100).toFixed(1)}%</span>
            </div>
          ))}
        </div>
        <div className="panel-source">Data: U.S. Census Bureau \u2014 PEP 2020\u20132023 &amp; ACS 1-Year 2023</div>
      </div>
    </div>
  )
}
